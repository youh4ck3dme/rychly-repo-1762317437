  ...
  try {
    switch (action) {
      case 'analyzeHair': {
        ...
        return json(res, 200, {
          currentHair: { color: 'Dark Brown', condition: 'Healthy', type: 'Wavy' },
          suggestions: [
            { name: 'Caramel Dimension', hairstyle: 'Layered', hex: '#B98555', description: 'Warm caramel layers enhance natural movement.', services: ['Farbenie (celých vlasov)'] },
            { name: 'Ash Gloss Upgrade', hairstyle: 'Current Style', hex: '#8A7F74', description: 'Soft neutral ash tone balances complexion.', services: ['Farbenie (odrasty)'] }
          ]
        });
      }
      case 'virtualTryOn': {
