...
const NavItem = React.memo(({ label, icon, active, onClick }: NavItemProps) => (
  <button
    onClick={onClick}
    className="flex flex-col items-center space-y-1 group focus:outline-none focus:ring-2 focus:ring-white rounded-md p-1 transition-transform transform active:scale-95 w-16"
  >
    <div
    ...
    <span
      className={`text-xs transition-colors ${active ? "text-white font-bold" : "text-gray-400 group-hover:text-white"}`}
    >
      {label}
    </span>
    <div
      className={`w-8 h-0.5 rounded-full transition-all duration-300 ${active ? "bg-white" : "bg-transparent group-hover:bg-gray-600"}`}
    />
  </button>
));
...
export const AppFooter = React.memo(
  ({ activeTab, onTabChange }: AppFooterProps) => {
