...
export const useHairstyle = () => {
    ...
    const applyHairstyle = useCallback(async (userImage: string, prompt: string, styleId: string) => {
        if (!userImage) {
        ...
        // If this is a refinement, don't clear the old image immediately for a smoother transition feel
        if (styleId !== 'refine') {
        ...
        if (result.data) {
            ...
            addNotification({ type: 'error', title: t('vto_error_title'), message: t(result.error) });
        }
