...
export const editImageWithSofia = async (base64ImageData: string, prompt: string, mimeType: string): Promise<{ data: string | null; error: string | null }> => {
    if (!isAiAvailable) {
    ...
    try {
        ...
        const response = await ai!.models.generateContent({
            ...
            contents: { // For this model, `contents` is a single Content object
                parts: [
                    {
                        inlineData: {
        ...
        if (response?.promptFeedback?.blockReason) {
            console.error("Request was blocked by Gemini API:", response.promptFeedback);
        ...
        console.warn("Gemini API did not return an image. Full response:", response);
