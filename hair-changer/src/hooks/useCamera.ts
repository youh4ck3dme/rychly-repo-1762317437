...
export const useCamera = (videoRef: RefObject<HTMLVideoElement>, canvasRef: RefObject<HTMLCanvasElement>) => {
        ...
        const enableStream = async () => {
            try {
                ...
                if (err instanceof DOMException) {
                    if (err.name === "NotAllowedError") errorMessage = 'vto_error_cameraPermission';
                    else if (err.name === "NotFoundError" || err.name === "NotReadableError") errorMessage = 'vto_error_cameraStart';
                }
                addNotification({ type: 'error', title: t('vto_error_title'), message: t(errorMessage) });
                stopCamera();
            }
        };
