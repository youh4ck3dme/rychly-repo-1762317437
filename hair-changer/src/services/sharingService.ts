...
const storage = getStorage(db.app); // Use db.app for initializing storage
 ...
 */
export const createShareSession = async (userImage: string): Promise<string | null> => {
    try {
 ...
 */
export const getShareSession = async (sessionId: string): Promise<string | null> => {
    try {
        const sessionRef = doc(db, 'sharedSessions', sessionId);
 ...
 */
export const uploadImageForSharing = async (base64Image: string): Promise<string | null> => {
    try {
