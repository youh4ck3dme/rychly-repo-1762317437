...
const analysisSchema = {
  ...
  properties: {
    currentHair: {
      ...
      properties: {
        color: {
          type: Type.STRING,
          description:
            "A descriptive name for the user's current hair color (e.g., 'Dark Brown', 'Ash Blonde').",
        },
        condition: {
          type: Type.STRING,
          description:
            "A brief assessment of the hair's condition (e.g., 'Healthy', 'Slightly Dry', 'Color-treated').",
        },
        type: {
          type: Type.STRING,
          description:
            "The hair type (e.g., 'Straight', 'Wavy', 'Curly', 'Coily').",
        },
      },
    ...
    suggestions: {
      type: Type.ARRAY,
      description:
        "An array of 3-4 recommended hair color and style suggestions.",
      items: {
        type: Type.OBJECT,
        properties: {
          name: {
            type: Type.STRING,
            description:
              "The name of the suggested look (e.g., 'Honey Blonde Wavy Lob').",
          },
          hairstyle: {
            type: Type.STRING,
            description:
              "The name of the hairstyle (e.g., 'Wavy Lob', 'Current Style').",
          },
          hex: {
            type: Type.STRING,
            description:
              "A valid CSS hex color code for the primary shade of the look (e.g., '#E6B883').",
          },
          description: {
            type: Type.STRING,
            description:
              "A concise, appealing description of the look, including WHY it suits the user's facial features.",
          },
          services: {
            type: Type.ARRAY,
            description:
              "A list of 2-3 specific salon service names from the provided list required to achieve this look. Use exact names from the list. If it is a custom color with no service, return ['N/A'].",
...
export const applyVirtualTryOn = async (
  imageDataUrl: string,
