...
export const HairHealthTracker: React.FC<HairHealthTrackerProps> = ({ isVisible, onClose }) => {
  ...
  const addNewEntry = () => {
    if (!newEntry.condition || !newEntry.hydration || !newEntry.shine || !newEntry.strength) return;
  ...
  const getAverageScore = () => {
    ...
    const avg = recent.reduce((acc, entry) =>
      acc + (entry.hydration + entry.shine + entry.strength) / 3, 0) / recent.length;
      ...
      >
        ...
        >
          ...
          <div className="bg-gradient-to-r from-amber-900/20 to-amber-800/20 rounded-xl p-6 mb-8 border border-amber-400/30">
            <h3 className="text-xl font-semibold text-amber-200 mb-4">Aktuálne skóre zdravia vlasov</h3>
            <div className="flex items-center gap-6">
              <div className="text-center">
                <div className={`text-4xl font-bold ${getConditionColor(
                  healthData.length > 0 ?
                  (getAverageScore() >= 8 ? 'excellent' :
                   getAverageScore() >= 6 ? 'good' :
                   getAverageScore() >= 4 ? 'fair' : 'poor') : 'good'
                )}`}>
                  {getAverageScore()}/10
                </div>
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div>
                    <div className="text-2xl font-bold text-blue-400">
                      {healthData.length > 0 ? Math.round(healthData.reduce((acc, d) => acc + d.hydration, 0) / healthData.length) : 5}
                    </div>
                    <div className="text-xs text-gray-400">Hydratácia</div>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-amber-400">
                      {healthData.length > 0 ? Math.round(healthData.reduce((acc, d) => acc + d.shine, 0) / healthData.length) : 5}
                    </div>
                    <div className="text-xs text-gray-400">Lesk</div>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-green-400">
                      {healthData.length > 0 ? Math.round(healthData.reduce((acc, d) => acc + d.strength, 0) / healthData.length) : 5}
                    </div>
          ...
          <div className="bg-gray-800/50 rounded-xl p-6 mb-8">
            <h3 className="text-xl font-semibold text-white mb-4">Pridať nový záznam</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Stav vlasov</label>
                <select
                  value={newEntry.condition}
                  onChange={(e) => setNewEntry({...newEntry, condition: e.target.value as HairHealthData['condition']})}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Hydratácia (1-10)</label>
                <input
                  type="range"
                  min="1"
                  max="10"
                  ...
                  className="w-full"
                />
                <div className="text-center text-amber-400 font-semibold">{newEntry.hydration}</div>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Lesk (1-10)</label>
                <input
                  type="range"
                  min="1"
                  max="10"
                  value={newEntry.shine}
                  onChange={(e) => setNewEntry({...newEntry, shine: parseInt(e.target.value)})}
                  className="w-full"
                />
                <div className="text-center text-amber-400 font-semibold">{newEntry.shine}</div>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Sila (1-10)</label>
                <input
                  type="range"
                  min="1"
                  max="10"
                  value={newEntry.strength}
                  onChange={(e) => setNewEntry({...newEntry, strength: parseInt(e.target.value)})}
                  className="w-full"
                />
                <div className="text-center text-amber-400 font-semibold">{newEntry.strength}</div>
              </div>
            </div>
          ...
          <div className="space-y-4">
            <h3 className="text-xl font-semibold text-white">História záznamov</h3>
            {healthData.length === 0 ? (
              <div className="text-center py-8 text-gray-400">
                ...
                <p>Žiadne záznamy</p>
                <p className="text-sm">Pridajte prvý záznam o zdraví vašich vlasov</p>
              </div>
            ) : (
                ...
                {healthData.map((entry, index) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      <div className={`px-3 py-1 rounded-full text-xs font-medium ${getConditionColor(entry.condition)}`}>
                        {entry.condition === 'excellent' ? 'Výborný' :
                         entry.condition === 'good' ? 'Dobrý' :
                         entry.condition === 'fair' ? 'Priemerný' : 'Slabý'}
                      </div>
                    ...
                    <div className="grid grid-cols-3 gap-4 text-sm">
                      <div className="text-center">
                        <div className="text-blue-400 font-semibold">{entry.hydration}/10</div>
                        <div className="text-gray-400">Hydratácia</div>
                      </div>
                      <div className="text-center">
                        <div className="text-amber-400 font-semibold">{entry.shine}/10</div>
                        <div className="text-gray-400">Lesk</div>
                      </div>
                      <div className="text-center">
                        <div className="text-green-400 font-semibold">{entry.strength}/10</div>
