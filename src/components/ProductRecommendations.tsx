    ...
    const generateRecommendations = () => {
      ...
      const productDatabase: Product[] = [
        {
          ...
          category: 'shampoo',
          benefits: ['Hĺbková výživa', 'Obnova poškodených vlasov', 'Lesk a hebkosť'],
        ...
        },
        {
          ...
          category: 'treatment',
          benefits: ['Obnova poškodených vlasov', 'Posilnenie', 'Prevencia lámavosti'],
        ...
        },
        {
          ...
          category: 'styling',
          benefits: ['Lesk a hebkosť', 'Ochrana pred teplom', 'Rýchle schnutie'],
      ...
      if (hairCondition === 'damaged') {
        filteredProducts = filteredProducts.filter(p =>
          p.suitableFor.includes('damaged') || p.benefits.some(b => b.includes('obnova') || b.includes('posilnenie'))
        );
      } else if (hairCondition === 'dry') {
        filteredProducts = filteredProducts.filter(p =>
          p.suitableFor.includes('dry') || p.benefits.some(b => b.includes('hydratácia') || b.includes('výživa'))
        );
      }
      ...
      if (selectedServices.some(s => s.toLowerCase().includes('farbenie'))) {
        filteredProducts = filteredProducts.filter(p =>
          p.category === 'color' || p.suitableFor.includes('colored')
        );
      }
        ...
        >
          ...
          {isLoading && (
            <div className="flex items-center justify-center py-20">
              <div className="text-center">
                ...
                <p className="text-amber-200 font-medium">AI analyzuje vaše vlasy...</p>
                <p className="text-gray-400 text-sm">Hľadáme tie najlepšie produkty pre vás</p>
              </div>
            </div>
          )}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="bg-amber-900/20 rounded-lg p-3 border border-amber-400/30">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-amber-200 text-sm font-medium">AI Match Score</span>
                        <span className="text-amber-400 font-bold">{product.matchScore}%</span>
                      </div>
                    ...
                    {/* Benefits */}
                    <div>
                      <h4 className="text-white font-medium text-sm mb-2">Výhody:</h4>
          ...
          {!isLoading && filteredProducts.length === 0 && (
            <div className="text-center py-20">
              ...
              <p className="text-gray-400 text-lg">Žiadne produkty v tejto kategórii</p>
              <p className="text-gray-500 text-sm">Skúste inú kategóriu alebo filtre</p>
            </div>
          )}
        </motion.div>
      </motion.div>
