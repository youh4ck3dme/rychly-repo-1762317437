...
import { Loader2, RefreshCw, Palette, ShieldCheck, Waves, Share2 } from 'lucide-react';
...
const SuggestionButton = React.memo(({ suggestion, onClick, isActive, children }: { suggestion?: HairSuggestion, onClick: () => void, isActive: boolean, children: React.ReactNode }) => (
    <motion.div className="relative" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
        <button
            ...
            style={suggestion ? { backgroundColor: suggestion.hex } : { backgroundColor: 'transparent' }}
...
export const ResultsScreen = React.memo(({ userImage, result, onRestart, setActiveServices, onTabChange }: ResultsScreenProps) => {
  const [selectedSuggestion, setSelectedSuggestion] = useState<HairSuggestion>(result.suggestions[0]);
  ...
  const [editedImagesCache, setEditedImagesCache] = useState<ImageCache>(() => {
    try {
      ...
      if (cachedData) {
        ...
        if (parsed && Array.isArray(parsed.order) && typeof parsed.items === 'object') {
          return parsed;
        }
      }
    } catch (error) {
      console.error("Failed to read virtual try-on cache from sessionStorage:", error);
  ...
  const [activeDisplayType, setActiveDisplayType] = useState<'suggestion' | 'original'>('suggestion');
  const [customColorSuggestion, setCustomColorSuggestion] = useState<HairSuggestion | null>(null);
  ...
  const updateImageCache = useCallback((key: string, value: string) => {
    setEditedImagesCache(prevCache => {
      ...
      try {
        sessionStorage.setItem(VIRTUAL_TRY_ON_CACHE_KEY, JSON.stringify(newCache));
      } catch (e) {
        console.error("Failed to write virtual try-on cache to sessionStorage:", e);
        // Fallback: If quota is still exceeded, try storing just the current item.
        if (e instanceof DOMException && e.name === 'QuotaExceededError') {
          console.warn("Quota exceeded. Clearing cache and attempting to save only the current item.");
          try {
            const singleItemCache: ImageCache = { order: [key], items: { [key]: value } };
            sessionStorage.setItem(VIRTUAL_TRY_ON_CACHE_KEY, JSON.stringify(singleItemCache));
            return singleItemCache;
          } catch (innerError) {
            console.error("Failed to write even a single item to session storage. Caching is disabled for this item.", innerError);
  ...
  const handleShare = useCallback(async () => {
    ...
    try {
        ...
        ctx.drawImage(beforeImg, padding, padding + labelHeight, imgWidth, imgHeight);
        ctx.drawImage(afterImg, imgWidth + padding * 2, padding + labelHeight, imgWidth, imgHeight);

        ctx.fillStyle = 'white';
        ctx.font = 'bold 24px -apple-system, sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText('Before', padding + imgWidth / 2, padding + labelHeight - 10);
        ctx.fillText('After', imgWidth + padding * 2 + imgWidth / 2, padding + labelHeight - 10);
  ...
  const handleSuggestionSelect = useCallback(async (suggestion: HairSuggestion) => {
  ...
  const handleApplyCustomColor = () => {
    ...
    // Sanitize hex input
    let cleanHex = customColorHex.startsWith('#') ? customColorHex : `#${customColorHex}`;
    ...
    const customSuggestion: HairSuggestion = {
        ...
        hex: cleanHex,
        description: 'A custom shade selected by you, applied to your current hairstyle.',
        services: ['N/A'],
    };
      ...
      <main className="flex-grow flex flex-col lg:flex-row lg:items-stretch">
           ...
           <AnimatePresence>
            {isGenerating && (
              ...
              >
                  ...
                  <p className="mt-4 text-sm font-semibold">{t('results.creatingLook')}</p>
              </motion.div>
            )}
           </AnimatePresence>
           {isColorPickerOpen && (
                ...
                >
                    <div className="bg-gray-800 p-6 rounded-lg w-full max-w-xs" onClick={(e) => e.stopPropagation()}>
                        <h3 className="font-bold text-lg text-center mb-4">{t('results.customColorTitle')}</h3>
                        <div className="flex items-center justify-center gap-4 mb-6">
                            <label htmlFor="colorPicker" className="sr-only">{t('results.customColorTitle')}</label>
                            <input
                                id="colorPicker"
                                name="colorPicker"
                                type="color"
                            ...
                            <div className="flex flex-col">
                                <label htmlFor="hexInput" className="text-xs text-gray-400 mb-1">{t('results.hexCode')}</label>
                                <input
                                    id="hexInput"
                                    name="hexInput"
                                    type="text"
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setIsColorPickerOpen(false)} className="w-full bg-gray-600 font-bold py-3 px-4 rounded-md hover:bg-gray-500 transition-colors">{t('results.cancel')}</button>
                            <button onClick={handleApplyCustomColor} className="w-full bg-white text-black font-bold py-3 px-4 rounded-md hover:bg-gray-200 transition-colors">{t('results.apply')}</button>
                        </div>
                    </div>
                </motion.div>
            )}
        ...
        <div className="p-4 flex-grow bg-gray-900 lg:w-1/2 lg:p-6 flex flex-col">
           <div className="lg:flex-grow lg:overflow-y-auto lg:pr-2">
                <h1 className="text-xl font-bold text-center tracking-wide">{t('results.title')}</h1>
                ...
                <div className="mt-4 p-4 bg-black rounded-lg">
                    ...
                    <ul className="text-sm text-gray-300 mt-2 space-y-2">
                        <li className="flex items-center gap-2">
                            ...
                            <span><strong>{t('results.currentColor')}:</strong> {result.currentHair.color}</span>
                        </li>
                        <li className="flex items-center gap-2">
                            <ShieldCheck size={16} className="text-yellow-300" />
                            <span><strong>{t('results.condition')}:</strong> {result.currentHair.condition}</span>
                        </li>
                        <li className="flex items-center gap-2">
                            <Waves size={16} className="text-yellow-300" />
                            <span><strong>{t('results.type')}:</strong> {result.currentHair.type}</span>
                        </li>
                    </ul>
                </div>
                ...
                <div className="mt-4">
                    ...
                    <div className="flex justify-center items-center flex-wrap gap-3 mt-3">
                        ...
                        <SuggestionButton
                            ...
                            isActive={activeDisplayType === 'suggestion' && selectedSuggestion.name.startsWith('Custom:')}
                        >
                        ...
                        {customColorSuggestion && (
                            <SuggestionButton
                                ...
                                onClick={() => handleSuggestionSelect(customColorSuggestion)}
                                isActive={activeDisplayType === 'suggestion' && selectedSuggestion.name === customColorSuggestion.name}
                            >
                                <div />
                            </SuggestionButton>
                        )}
                        {result.suggestions.map((suggestion) => (
                             <SuggestionButton
                                ...
                                isActive={activeDisplayType === 'suggestion' && selectedSuggestion.name === suggestion.name}
                            >
                                <div />
                            </SuggestionButton>
                        ))}
                ...
                <div className="mt-4 p-4 bg-black rounded-lg">
                    <h3 className="font-bold">{activeDisplayType === 'suggestion' ? selectedSuggestion.name : t('results.originalHair')}</h3>
                    {activeDisplayType === 'suggestion' && <p className="text-sm font-semibold text-gray-400 mt-1">{t('results.hairstyle')}: {selectedSuggestion.hairstyle}</p>}
                    <p className="text-sm text-gray-300 mt-1">{activeDisplayType === 'suggestion' ? selectedSuggestion.description : t('results.originalHairDescription')}</p>
                    {activeDisplayType === 'suggestion' && selectedSuggestion.services.length > 0 && selectedSuggestion.services[0] !== 'N/A' && (
                        <>
                        <p className="text-xs text-gray-400 mt-2">{t('results.recommendedServices')}: {selectedSuggestion.services.join(', ')}</p>
                        <button
