...
export const ServicesScreen = React.memo(({ activeServices }: ServicesScreenProps) => {
  const { t } = useTranslation();
  return (
    <div className="flex flex-col flex-grow bg-black text-white">
      <main className="flex-grow flex flex-col p-4">
        <h1 className="text-3xl font-bold text-center mb-8 tracking-wide">{t('services.title')}</h1>
        <motion.div 
            className="space-y-8"
                ...
                return (
                    ...
                    >
                        ...
                        <div className="relative p-6">
                            <h3 className="text-3xl font-bold text-yellow-300 mb-6 tracking-wider" style={{ textShadow: '0 2px 10px rgba(255, 215, 0, 0.3)' }}>
                                {t(category.nameKey)}
                            </h3>
                            <motion.div
                            ...
                            >
                                {category.subcategories.map((subcategory) => (
                                    <motion.div
                                        ...
                                        whileHover={{ y: -5 }}
                                    >
                                        <h4 className="font-bold text-lg mb-3 text-white tracking-wide">{subcategory.name}</h4>
                                        <ul className="space-y-3">
                                            {subcategory.items.map((item) => {
                                                const isRecommended = activeServices.includes(item.name);
                                                return (
                                                    <li key={item.name} className={`flex justify-between items-center transition-all border-b border-gray-800 pb-2 last:border-b-0 ${isRecommended ? 'text-white' : 'text-gray-300'}`}>
                                                        <div className="flex items-center gap-2">
                                                             {isRecommended && <Star size={14} className="text-yellow-400 fill-yellow-400" />}
                                                             <span className={`${isRecommended ? 'font-bold' : ''}`}>{item.name}</span>
                                                        </div>
                                                        <span className={`font-semibold ${isRecommended ? 'text-yellow-300' : 'text-gray-400'}`}>{item.price}</span>
                                                    </li>
                                                );
                                            })}
                                        </ul>
