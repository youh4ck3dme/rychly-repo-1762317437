...
const ChatMessageBubble = React.memo(({ message }: ChatMessageBubbleProps) => {
    ...
    return (
        <div className={`flex items-start gap-3 ${isModel ? '' : 'justify-end'}`}>
            ...
            <div className={`px-4 py-2 rounded-2xl max-w-xs md:max-w-md ${isModel ? 'bg-gray-700 rounded-bl-none' : 'bg-white text-black rounded-br-none'}`}>
...
export const Chatbot = React.memo(({ isOpen, onClose, messages, onSendMessage, isLoading }: ChatbotProps) => {
      ...
      {isOpen && (
        ...
        >
          ...
          <header className="flex items-center justify-between p-4 border-b border-gray-800 flex-shrink-0">
            ...
            <button onClick={onClose} className="p-1 rounded-full hover:bg-gray-800" aria-label={t('chatbot.close')}>
              <X size={20} />
            </button>
          </header>
