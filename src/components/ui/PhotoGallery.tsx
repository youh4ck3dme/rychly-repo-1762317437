...
export const PhotoGallery = React.memo(
  ({ photos, onSelectPhoto }: PhotoGalleryProps) => {
    ...
    return (
      <div className="w-full max-w-sm mx-auto mt-4">
        <h3 className="text-xs text-center text-gray-400 mb-2 uppercase tracking-wider">
          {t("upload.recentPhotos")}
        </h3>
        <div className="grid grid-cols-4 gap-2">
          {photos.map((photo, index) => (
            ...
            >
              <img
                src={photo}
                alt={`${t("upload.recentPhotoAlt")} ${index + 1}`}
