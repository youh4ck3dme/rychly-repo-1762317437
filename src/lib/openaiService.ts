...
${context ? `Context: ${context}` : ''}`;
  ...
  /**
     ...
     try {
       ...
       const response = await this.client.chat.completions.create({
         ...
         messages: [
           {
             role: "system",
             content: "You are a professional hair stylist providing specific style recommendations."
           },
           {
             role: "user",
             content: prompt
    ...
    */
   async generateVirtualTryOn(userImage: string, suggestion: any): Promise<{
     image: string;
     confidence: number;
    ...
    */
   async generateHairPreview(imageUrl: string, style?: string, color?: string): Promise<{
     image: string;
     confidence: number;
     processingTime: number;
     suggestions?: string[];
