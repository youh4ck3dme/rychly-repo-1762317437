...
export function secureAPI(
  ...
  const config = {
    ...
    csp:
      options.csp ||
      "default-src 'none'; frame-ancestors 'none'; base-uri 'none'; form-action 'none'; img-src 'self' data: https:;",
    cors: {
  ...
  return async (ctx: { request: Request }) => {
    ...
    // Honeypot protection
    if (
      config.honeypotField &&
      typeof body[config.honeypotField] === "string"
    ) {
    ...
    if (
      !rateLimitData ||
      currentTime - rateLimitData.ts > config.rateLimitWindowMs
    ) {
