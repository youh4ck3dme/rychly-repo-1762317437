...
export const POST: APIRoute = secureAPI(async (request: Request, sanitizedBody: HairAnalysisRequest) => {
  ...
  const generateFallbackResponse = (reason: string): HairAnalysisResponse => ({
    ...
    recommendations: [
      {
        ...
        description: "Moderný strih, ktorý dodá objem a dynamiku vašim vlasom. Vhodný pre väčšinu typov tváre.",
        difficulty: "medium",
        maintenance: "medium"
      },
      {
        ...
        description: "Jemné melírovanie v teplých odtieňoch, ktoré rozjasní váš vzhľad a dodá rozmer.",
        difficulty: "easy",
        maintenance: "low"
      },
      {
        ...
        description: "Prírodný styling s použitím texturizačného spraya pre ležérny, ale upravený vzhľad.",
        difficulty: "easy",
        maintenance: "low"
      },
      {
        ...
        description: "Týždenná hydratačná maska pre zachovanie zdravých a lesklých vlasov.",
        difficulty: "easy",
        maintenance: "medium"
      }
    ],
  ...
  try {
    ...
    if (!process.env.OPENAI_API_KEY || process.env.OPENAI_API_KEY.includes('your-') || process.env.OPENAI_API_KEY.trim() === '' || !process.env.OPENAI_API_KEY.startsWith('sk-')) {
      return new Response(JSON.stringify(generateFallbackResponse("OpenAI API key not configured")), {
        status: 200,
        headers: { 'Content-Type': 'application/json' }
      });
    }
    ...
    const parsedResponse: HairAnalysisResponse = {
      ...
      recommendations: analysis.recommendations.map((rec: string, index: number) => ({
        type: index % 4 === 0 ? "strih" : index % 4 === 1 ? "farba" : index % 4 === 2 ? "styling" : "starostlivost",
    ...
    return new Response(JSON.stringify(generateFallbackResponse(`Service temporarily unavailable: ${error.message}`)), {
      status: 200,
      headers: { 'Content-Type': 'application/json' }
    });
  }
  ...
  image: {
    allowHosts: ['imgur.com', 'cloudinary.com', 'unsplash.com', 'pexels.com', 'i.ibb.co', 'postimg.cc'],
