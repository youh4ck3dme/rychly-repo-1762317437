...
export const POST: APIRoute = secureAPI(
  async (request: Request, sanitizedBody: HairPreviewRequest) => {
    ...
    try {
      ...
      if (
        ...
        const fallbackPreview = await generateFallbackPreview(
          imageUrl,
          style,
          color,
        );
      ...
      const previewResult = await openaiService.generateHairPreview(
        imageUrl,
        style,
        color,
      );
