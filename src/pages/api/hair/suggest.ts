...
export const POST: APIRoute = secureAPI(
  async (request: Request, sanitizedBody: HairSuggestRequest) => {
    const {
      hairType,
      faceShape,
      preferences,
    ...
    try {
      ...
      const colorSuggestions = await generateColorSuggestions(
        preferences,
        hairType,
      );

      // Generate care suggestions
      const careSuggestions = await generateCareSuggestions(
        hairType,
        preferences,
      );
      ...
      return new Response(
        JSON.stringify({
          suggestions: fallbackSuggestions.map((content) => ({
            type: "style",
            content,
          })),
          confidence: 0.3,
  ...
  const suggestions = [
    {
      type: "style",
      content:
        "Vrstvený strih strednej dĺžky s jemnými vlnami pre prirodzený vzhľad",
    },
    {
      type: "color",
...
async function generateCareSuggestions(
  hairType?: string,
  preferences?: any,
): Promise<string[]> {
  const baseCare = [
